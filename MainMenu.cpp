...
void MainMenu::outputOldUserFile(string userName) {
	string	fileName = "AllUserNames/" + userName + ".csv";
        ...
        if(!file.is_open()){
        	...
        	cout << userName << "'s profile has been created instead." << endl;
	}
        else {		
...
string MainMenu::getPrintedList(vector<Task*> toPrint) {
	...
	if(toPrint.at(i)->getDay() == 99 && toPrint.at(i)->getMonth() == 99 && toPrint.at(i)->getYear() == 9999) {
	dueDateExists = false;
	}
...
void MainMenu::editDate() {
 ...
 //   	newTask->setDay(day);
  ...
  //  	newTask->setYear(year);
	...
	cin.ignore();
	cout << "Okay. Date for " << userTitle << " has been successfully changed." << endl;
	printVector(dates);
	...
	if (input == "n") { 
		...
		for (int i = 0; i < dates.size(); i++) {
                	if (dates.at(i)->getTitle() == userTitle) {
                        	dates.at(i)->setMonth(00);
                        	dates.at(i)->setDay(00);
                        	dates.at(i)->setYear(0000);
                        	index = i;
               		}
                }
